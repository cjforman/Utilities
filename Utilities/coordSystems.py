...
def cyl2XYZ(pos, axis=np.array([0.0, 0.0, 1.0]), basePoint=np.array([0.0, 0.0, 0.0]), zeroPoint=np.array([1.0, 0.0, 0.0])): # rho, phi (az), z to x, y, z
    if False in np.concatenate((axis==np.array([0.0, 0.0, 1.0]), basePoint==np.array([0.0, 0.0, 0.0]), zeroPoint==np.array([1.0, 0.0, 0.0]))):
        nHat, zeroPhiVecHat, posPhiVecHat = cylBasis(axis=axis, basePoint=basePoint, zeroPoint=zeroPoint)    
        return basePoint + pos[0] * np.cos(pos[1]) * zeroPhiVecHat + pos[0] * np.sin(pos[1]) * posPhiVecHat +pos[2] * nHat     
    else:
...
def XYZ2Cyl(pos, axis=np.array([0.0, 0.0, 1.0]), basePoint=np.array([0.0, 0.0, 0.0]), zeroPoint=np.array([1.0, 0.0, 0.0])):# x, y, z to rho, phi, z

    if False in np.concatenate((axis==np.array([0.0, 0.0, 1.0]), basePoint==np.array([0.0, 0.0, 0.0]), zeroPoint==np.array([1.0, 0.0, 0.0]))):
        nHat, zeroPhiVecHat, posPhiVecHat = cylBasis(axis=axis, basePoint=basePoint, zeroPoint=zeroPoint)
    else:
        nHat = axis
        zeroPhiVecHat = zeroPoint 
        posPhiVecHat = np.array([0.0, 1.0, 0.0])        
        
    # Get the z-parameter for the position point along the axis
    zVal = np.dot(pos - basePoint, nHat)
    
    # get the vector in the radial direction
    rhoVec = pos - zVal * axis - basePoint
    ...
    if rho > 0.0000001:
        ...
        # if rho is zero then phi is undefined so just pick a direction for a unit radial vector.
        rhoHat = zeroPhiVecHat
        
    # phi is azimuthal angle between rhoHat and zeroPhiVecHat, 
    phi = np.arccos( np.dot( rhoHat, zeroPhiVecHat ) )
    
    # check sign - positive phi is in positive y half-space and vice versa
    if np.dot(posPhiVecHat, rhoHat) < 0.0 :
        phi = -phi
    
    return np.array([rho, phi, zVal])

def azToRhoCyl(az, r_x, r_y): 
    # returns the distance from origin to an ellipse at a particular azimuthal argument (in radians), 
    # when the ellipse has semi axis r_x on x axis and r_y on y_axis.zero azimuth is along x_axis.
    # Principal ellipse axes are aligned with the X and Y axes.
    a = np.sin(az)/r_x
    b = np.cos(az)/r_y
    p = a**2 + b**2
...
def axisFromHelix(xyzList):
    ...
    To understand this better:  The tangent vectors are the vectors between successive points in the list. 
    Any two adjacent tangents form a plane (i.e., 3 successive points in the list).  The bivector bisects the 
    internal angle between the tangents in the plane of the two tangents.  The cross product of two adjacent 
    bivectors (requires 4 input points in total) should yield a vector that is approximately aligned with the 
    axis of a helix if the original points are on a helix.  We take the mean of all such "axial vectors" 
    for the given set of points. This is a greedy algorithm and will always return an axis for a given set of points.
    This axis may be a lot of twaddle for a random set of points, but if the input points are close 
    to being a real helix (organised in order along the helix), this will be a reasonable estimate of the 
    axial vector. Up to the user to use this responsibly.   
    The output vector is normalised to 1.0. '''
    ...
    if len(xyzList)==3:
        return (xyzList[2] - xyzList[0])/np.linalg.norm((xyzList[2] - xyzList[0])) 
...
def SphereArea(phiMin, phiMax, thetaMin, thetaMax, radius):
    return radius * radius * ( phiMax - phiMin ) * ( np.sin( thetaMax ) - np.sin( thetaMin ) )
...
def rotatePointInChain(points, angle):
    ...
    # by the given angle. Conserves the distance between the points and bond angle but may change the bond angles in a chain.
...
def pickRandomPointInSphericalRange(r1, r2, theta1, theta2, phi1, phi2):
    ...
    x = r * rMax * np.cos(theta)*np.cos(phi)
    y = r * rMax * np.cos(theta)*np.sin(phi)
    z = r * rMax * np.sin(theta)
    return np.array([x, y, z])


def pickRandomPointInEllipsoidRange(rx, ry, rz, theta1, theta2, phi1, phi2):
    # picks a direction within an angular range in the unit sphere and computes the maximum radius in that direction that lies within an ellipsoid.
    # then picks a radius randomly that is along that line. with a lower probability of choosing on closer to the centre of the sphere. 
    rFrac = np.sqrt(rnd.uniform(0.0, 1.0))
    ...
    # find the distance to the ellipsoid and the unit vector in that direction
    r, rVec = ellipsoidalPolarUVW(theta, phi, rx, ry, rz)

    #x = rx * r * np.cos(theta)*np.cos(phi)
    #y = ry * r * np.cos(theta)*np.sin(phi)
    #z = rz * r * np.sin(theta)
    # return the vector scaled by random magnitude between 0 and 1
    return rVec * rFrac * r
...
if __name__== '__main__':
    ...
    print( "Cylindrical default basis" )
    ...
    print( "Cylindrical not default basis" )

    aCyl = XYZ2Cyl(a, axis=np.array([0.0, 1.0, 0.0]), basePoint=np.array([1.0,1.0,1.0]), zeroPoint=np.array([1.0, 0.0, 0.0]) )
    bCyl = XYZ2Cyl(b, axis=np.array([0.0, 1.0, 0.0]), basePoint=np.array([1.0,1.0,1.0]), zeroPoint=np.array([1.0, 0.0, 0.0]))
    cCyl = XYZ2Cyl(c, axis=np.array([0.0, 1.0, 0.0]), basePoint=np.array([1.0,1.0,1.0]), zeroPoint=np.array([1.0, 0.0, 0.0]))
    dCyl = XYZ2Cyl(d, axis=np.array([0.0, 1.0, 0.0]), basePoint=np.array([1.0,1.0,1.0]), zeroPoint=np.array([1.0, 0.0, 0.0]))
    eCyl = XYZ2Cyl(e, axis=np.array([0.0, 1.0, 0.0]), basePoint=np.array([1.0,1.0,1.0]), zeroPoint=np.array([1.0, 0.0, 0.0]))
    fCyl = XYZ2Cyl(f, axis=np.array([0.0, 1.0, 0.0]), basePoint=np.array([1.0,1.0,1.0]), zeroPoint=np.array([1.0, 0.0, 0.0]))
    gCyl = XYZ2Cyl(g, axis=np.array([0.0, 1.0, 0.0]), basePoint=np.array([1.0,1.0,1.0]), zeroPoint=np.array([1.0, 0.0, 0.0]))
    hCyl = XYZ2Cyl(h, axis=np.array([0.0, 1.0, 0.0]), basePoint=np.array([1.0,1.0,1.0]), zeroPoint=np.array([1.0, 0.0, 0.0]))

    aXYZCyl = cyl2XYZ(aCyl, axis=np.array([0.0, 1.0, 0.0]), basePoint=np.array([1.0,1.0,1.0]), zeroPoint=np.array([1.0, 0.0, 0.0]))
    bXYZCyl = cyl2XYZ(bCyl, axis=np.array([0.0, 1.0, 0.0]), basePoint=np.array([1.0,1.0,1.0]), zeroPoint=np.array([1.0, 0.0, 0.0]))
    cXYZCyl = cyl2XYZ(cCyl, axis=np.array([0.0, 1.0, 0.0]), basePoint=np.array([1.0,1.0,1.0]), zeroPoint=np.array([1.0, 0.0, 0.0]))
    dXYZCyl = cyl2XYZ(dCyl, axis=np.array([0.0, 1.0, 0.0]), basePoint=np.array([1.0,1.0,1.0]), zeroPoint=np.array([1.0, 0.0, 0.0]))
    eXYZCyl = cyl2XYZ(eCyl, axis=np.array([0.0, 1.0, 0.0]), basePoint=np.array([1.0,1.0,1.0]), zeroPoint=np.array([1.0, 0.0, 0.0]))
    fXYZCyl = cyl2XYZ(fCyl, axis=np.array([0.0, 1.0, 0.0]), basePoint=np.array([1.0,1.0,1.0]), zeroPoint=np.array([1.0, 0.0, 0.0]))
    gXYZCyl = cyl2XYZ(gCyl, axis=np.array([0.0, 1.0, 0.0]), basePoint=np.array([1.0,1.0,1.0]), zeroPoint=np.array([1.0, 0.0, 0.0]))
    hXYZCyl = cyl2XYZ(hCyl, axis=np.array([0.0, 1.0, 0.0]), basePoint=np.array([1.0,1.0,1.0]), zeroPoint=np.array([1.0, 0.0, 0.0]) )
